...
export default function DocumentsPage() {
  ...
  const [modal, setModal] = useState<null | "upload" | "history" | "compare">(
    null,
  );

// Pull locally created documents into the table until backend storage is added.
  useEffect(() => {
    ...
    const savedDocuments: DocumentItem[] = parsedDocuments.map(
      (item: {
        id: number;
        title: string;
        documentType: string;
        ...
        id: item.id,
        type: item.title || item.documentType,
        version: item.currentVersion || item.version || "v1",
        updated: "Today",
        status: item.status || "Draft",
        reviewer: item.status === "Submitted" ? "Adviser" : "Not submitted",
      }),
    );

    setDocuments([...savedDocuments, ...initialDocuments]);
  }, []);
                ...
                <tr key={document.id} className="hover:bg-[#fbfdfb]">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex flex-wrap gap-2">
                      <Link
                        href={`/student/documents/${document.id}/edit`}
              ...
              <div className="mt-5 space-y-4">
                <div className="rounded-xl border border-dashed border-[#dfe8df] bg-[#f8faf7] p-6 text-center">
                  ...
                  <p className="mt-1 text-xs text-[#7b877f]">
                    In deployment, this connects to file storage and creates a
                    new version record.
                  </p>
                </div>
